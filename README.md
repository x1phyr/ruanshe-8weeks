...
**53 日内容已全部就绪**：49 日 `status: live`（完整讲义 + 练习题；**非试卷日 ≥12 题**），4 日 `status: paper`（150 分钟计时壳 + 用法指导 + 站内自编模考：上午各 **75** 题、下午各 **20** 道案例风格单选；**不**提供受版权保护的真题/案例原文）。正式卷面书写请自备合法材料。进度键仍是 `week-N/day-M`。
