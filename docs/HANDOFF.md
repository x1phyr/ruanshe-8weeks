...
预期输出：`{ total: 784, daily: 594, mock: 190 }`（`questions` 数组末尾 `...mockPaperQuestions`，190 = 75+20+75+20）。

约束：**非试卷日 ≥12 题**（试卷日保持模考题量 75/20/75/20，不削减）。
...
4. 题量与覆盖度复查：当前全站 784 题（非试卷日 ≥12）；可按模块正确率与错题热点定向加题。
